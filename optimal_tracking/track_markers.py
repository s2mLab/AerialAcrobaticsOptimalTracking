...
if __name__ == "__main__":
    ...
    if initial_guess_track_Q:
        ...
        q_ref = sol_track_q.states['q']
        qdot_ref = sol_track_q.states['qdot']
        tau_ref = sol_track_q.controls['tau'][:, :-1]
    elif use_optimized_gravity:
        track_q_gravity_filename = 'solutions/track_q_gravity/' + os.path.splitext(c3d_name)[0] + '_N' + str(adjusted_number_shooting_points) + ".bo"
        ocp_track_q_gravity, sol_track_q_gravity = OptimalControlProgram.load(track_q_gravity_filename)
        q_ref = sol_track_q_gravity.states['q']
        qdot_ref = sol_track_q_gravity.states['qdot']
        tau_ref = sol_track_q_gravity.controls['tau'][:, :-1]
        params_track_q_gravity = sol_track_q_gravity.parameters

        # --- Modify gravity --- #
        angle = params_track_q_gravity["gravity_angle"].squeeze()
    ...
    # --- Save --- #
    save_path = 'solutions/track_markers/'
    ...
    with open(save_variables_name, 'wb') as handle:
        pickle.dump({'mocap': markers_reordered, 'duration': duration, 'frames': frames, 'step_size': step_size,
                     'q': q_sol, 'qdot': qdot_sol, 'tau': tau_sol, 'gravity': gravity},
                    handle, protocol=3)
